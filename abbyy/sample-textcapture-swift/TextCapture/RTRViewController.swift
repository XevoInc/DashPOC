...
class RTRViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    /// String used for text search/match
    var searchString = ""
...
//# MARK: - LifeCycle
	...
	override func viewDidLoad()
	{
        ...
        //Custom
        self.recognizeLanguageButton.title = "Search"
...
//# MARK: - Drawing result
	...
	func drawTextLine(_ textLine: RTRTextLine, _ layer: CALayer, _ progress: RTRResultStabilityStatus)
	{
        ...
        //Custom
        if (progress == RTRResultStabilityStatus.stable)
        {
            self.searchText(recognizedString: recognizedString!, rectForTextLayer: rectForTextLayer, angle: angle, position: bottomLeft)
        }
	}
    ...
    //# MARK: - Custom Text Search/Match
    
    func searchText(recognizedString: String, rectForTextLayer: CGRect, angle: CGFloat, position: CGPoint)  {
        
        print("\(String(describing: recognizedString))")
        if ((recognizedString.lowercased().range(of: self.searchString.lowercased())) != nil)
        {
            self.performBlockOnMainThread(0, closure: {
                self.enableTextButton(rect: rectForTextLayer, angle: angle, position: position)
            })
        }
    }
    
    func enableTextButton(rect: CGRect, angle: CGFloat, position: CGPoint) {
        ...
        button.layer.transform = CATransform3DRotate(CATransform3DIdentity, angle, 0, 0, 1)
