...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    func didPinch(_ gesture: UIPinchGestureRecognizer) {
        switch gesture.state {
        case .began:
            var tapPoint = gesture.location(ofTouch: 1, in: sceneView)
            var result = sceneView.hitTest(tapPoint, options: nil)
            if (result.count == 0)
            {
                tapPoint = gesture.location(ofTouch: 0, in: sceneView)
            ...
            trackedObject = hitResult?.node.parent as? VirtualObject
        case .changed:
